...
sub_category = '1 string replaced by number'
entity_type = 'Entity'
...
def replace_string_with_number(name):
...
for index, row in ofac_list_sampled.iterrows():
    ...
    # replace 1 string with a number in the name
    modified_name = replace_string_with_number(row['name']) 
    
    # construct the test case name
    final_test_name = modified_name.upper()
    final_test_cases.loc[len(final_test_cases)] = [uid, theme, category, sub_category, entity_type, uid + ' - ' + str(index), row['uid'], row['name'], final_test_name] # append to the dataframe
    
final_test_cases.to_csv('final_test_cases_159.csv', index=False, columns=['UID', 'Theme', 'Category', 'Sub-category', 'Entity-Type', 'Original Name', 'Test Case Name'])
