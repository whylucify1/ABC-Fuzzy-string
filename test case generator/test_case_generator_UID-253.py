...
for index, row in ofac_list_sampled.iterrows():
    ...
    if number_of_words_first_middle == 2:
     ...
     c0=separated_words[0][0]
     c1=separated_words[0][1]
     c2=separated_words[0][2]
     transp = c2+c0+c1+separated_words[0][3:]
     final_test_name = transp + ' ' +  separated_words[1] + ' ' + separated_words[2] #create the test case
     ...
     final_test_cases.loc[len(final_test_cases)] = [uid, theme, category, sub_category, entity_type, uid + ' - ' + str(index), row['uid'], row['name'], final_test_name.upper()] # append to the dataframe
        ...
        c2=separated_words[0][2]
        transp = c2+c0+c1+separated_words[0][3:]
        final_test_name = transp + ' ' +  separated_words[1] #create the test case
        print(final_test_name)
        final_test_cases.loc[len(final_test_cases)] = [uid, theme, category, sub_category, entity_type, uid + ' - ' + str(index), row['uid'], row['name'], final_test_name.upper()] # append to the dataframe
