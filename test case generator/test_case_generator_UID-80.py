...
ofac_list_filtered = ofac_list_filtered[ofac_list_filtered['word_count'] > 3]
...
def remove_middle_names(name):
    """
    Removes up to three middle names by splitting the name into parts and
    removing the middle parts.
    ...
    # randomly select three indices to remove
    indices = sorted(random.sample(range(1, num_parts-1), 3), reverse=True)
...
for index, row in ofac_list_sampled.iterrows():
    ...
    final_test_name = remove_middle_names(busdes_name)
...
final_test_cases[['Sub-category', 'Original Name', 'Test Case Name']].to_csv('test_cases_80.csv', index=False, header=['Test Type', 'Original Name', 'Altered Name'])

row_count = ofac_list_sampled.shape[0]
print("Number of rows in ofac_list_sampled:", row_count)
