...
category = 'Title'
sub_category = '1 Title Added Twice'
...
def rand_title():
    ...
    rand_title = titles[random.randrange(1,5)]
    return rand_title
...
import pandas as pd
from test_case_generator_functions import separate_words_in_name_3, separate_words_in_name_2, rand_title 
...
for index, row in ofac_list_sampled.iterrows():
    ...
    if number_of_words_first_middle == 1:
        ...
        final_test_name = (rand_title() + ' ')*2 + ' ' + separated_words[0] + ' ' +  separated_words[1] #create the test case
        ...
        final_test_name = (rand_title() + ' ')*2 + ' ' + separated_words[0] + ' ' + separated_words[1] + ' ' + separated_words[2]  # create the test case
        final_test_cases.loc[len(final_test_cases)] = [uid, theme, category, sub_category, entity_type, uid + ' - ' + str(index), row['uid'], row['name'], final_test_name.upper()] # append to the dataframe
        print(final_test_name)