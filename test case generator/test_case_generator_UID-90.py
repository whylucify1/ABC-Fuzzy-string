...
def replace_title(text, titles):
    ...
    if identified_title:
        return text.replace(identified_title, '').strip()
    return text

def remove_title(name, titles):
    for title in titles:
        if name.upper().startswith(title.upper()):
...
for index, row in ofac_list_sampled.iterrows():
    ...
    final_test_name = remove_title(final_test_name, titles) # remove identified titles from the name
