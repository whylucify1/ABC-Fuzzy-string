...
category = 'First name'
sub_category = '1 first name added twice'
...
for index, row in ofac_list_sampled.iterrows():
    ...
    if number_of_words_first_middle != 1:
        ...
        final_test_name = separated_words[0] + ' ' + separated_words[0] + ' ' + separated_words[1]
        final_test_cases.loc[len(final_test_cases)] = [uid, theme, category, sub_category, entity_type, uid + ' - ' + str(index), row['uid'], row['name'], final_test_name] # append to the dataframe
        print(final_test_cases)
    else: 
        separated_words = separate_words_in_name_3(row['name'])
        final_test_name = separated_words[0] + ' ' + separated_words[0] + ' ' + separated_words[1] + ' '  + separated_words[2]
        final_test_cases.loc[len(final_test_cases)] = [uid, theme, category, sub_category, entity_type, uid + ' - ' + str(index), row['uid'], row['name'], final_test_name] # append to the dataframe
        print(final_test_cases)
