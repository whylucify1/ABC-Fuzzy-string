...
for index, row in ofac_list_sampled.iterrows():
    ...
    name1= last_word
    index2 = random.randint(0, len(name1)-1)
    while not name0[index1].isalpha():
        index1 = random.randint(0, len(name0)-1)

    modified_name1 = name1[:index2] + str(random.randint(0,9)) + name1[index2+1:] 
    ########
    
    final_test_name = " ".join([modified_name0] + middle_words + [modified_name1])
