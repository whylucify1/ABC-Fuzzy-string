...
theme = 'typo'
category = 'Full Name'
sub_category = 'First / last name swapped'
...
for index, row in ofac_list_sampled.iterrows():
    ...
    if number_of_words_first_middle == 1:
        ...
        print(separated_words[1])
        c0=separated_words[1][0]
        c1=separated_words[1][1]
        typo=c1+c0+separated_words[1][2:]
        print(typo)
        final_test_name=separated_words[0] + ', ' + typo
        ...
        #final_test_name = separated_words[0] + ', ' + separated_words[2] + ' ' + separated_words[1] # create the test case
        #
        #final_test_cases.loc[len(final_test_cases)] = [uid, theme, category, sub_category, entity_type, uid + ' - ' + str(index), row['uid'], row['name'], final_test_name] # append to the dataframe
        pass