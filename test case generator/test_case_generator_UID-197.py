...
for index, row in ofac_list_sampled.iterrows():
    ...
    if number_of_words_first_middle != 1:
        ...
        index1 = random.randint(0, len(name)-1)
        index2 = random.randint(0, len(name)-1)

        # make sure that the two indices are not equal
        while index2 == index1:
              index2 = random.randint(0, len(name)-1)

        modified_name1 = name[:index1] + str(random.randint(0,9)) + str(random.randint(0,9)) + name[index2+1:]
        
     
     
        name = separated_words[0] 
        index1 = random.randint(0, len(name)-1)
   

        modified_name0 = name[:index1] + str(random.randint(0,9)) + str(random.randint(0,9)) + name[index2+1:]
        ...
        final_test_name = modified_name0 + ' ' + modified_name1 + ' ' + separated_words[2] + '.'  #create the test case        
        ...
        final_test_name = modified_name0+ ' ' + modified_name1 + '.'  #create the test case        
        final_test_cases.loc[len(final_test_cases)] = [uid, theme, category, sub_category, entity_type, uid + ' - ' + str(index), row['uid'], row['name'], final_test_name.upper()] # append to the dataframe
        print(final_test_name)
