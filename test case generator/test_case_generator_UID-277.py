...
for index, row in ofac_list_sampled.iterrows():
    ...
    if number_of_words_first_middle == 2:
     ...
     final_test_name = separated_words[0][1:] + ' ' + separated_words[1] + ' ' + separated_words[2]
     ...
     final_test_cases.loc[len(final_test_cases)] = [uid, theme, category, sub_category, entity_type, uid + ' - ' + str(index), row['uid'], row['name'], final_test_name] # append to the dataframe
        ...
        final_test_name = separated_words[0][1:] + ' ' + separated_words[1]
        print(final_test_name)
        final_test_cases.loc[len(final_test_cases)] = [uid, theme, category, sub_category, entity_type, uid + ' - ' + str(index), row['uid'], row['name'], final_test_name] # append to the dataframe

#%%

final_test_cases.to_csv('new csv files/277.csv', index=False)   