...
for index, row in ofac_list_sampled.iterrows():
    ...
    if number_of_words_first_middle == 2:
        ...
        # check if separated_words has enough elements
        if len(separated_words) < 3:
            continue
        
        # get initials for the middle names
        middle_initials = '.'.join([word[0].upper() for word in separated_words[1:-1]])
        
        # construct the test case name
        final_test_name = separated_words[-1] + ' ' + middle_initials + ' ' + separated_words[0][0] + '.'
