...
uid = 'UID-93' 
theme = 'Given name and last name swaped'
category = 'Full name'
sub_category = 'Given name and last name swaped with no comma'
...
for index, row in ofac_list_sampled.iterrows():
    ...
    if number_of_words_first_middle == 1:
        ...
        final_test_name = separated_words[0] + separated_words[1] + ' ' + separated_words[2] # Create test cases
        final_test_cases.loc[len(final_test_cases)] = [uid, theme, category, sub_category, entity_type, uid + ' - ' + str(index), row['uid'], row['name'], final_test_name.upper()]
        print(final_test_cases)
