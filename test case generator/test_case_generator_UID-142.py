...
category = 'initials'
sub_category = 'First Name Initial'
...
for index, row in ofac_list_sampled.iterrows():
    ...
    if number_of_words_first_middle > 1:
        ...
        final_test_name = abbreviated_first_name + '.' + ' ' + separated_words[1] + ' ' + separated_words[2]  #create the test case        
        ...
        abbreviated_first_name = separated_words[0][0]
        final_test_name = abbreviated_first_name + '.' + ' ' + separated_words[1]  #create the test case        
        final_test_cases.loc[len(final_test_cases)] = [uid, theme, category, sub_category, entity_type, uid + ' - ' + str(index), row['uid'], row['name'], final_test_name.upper()] # append to the dataframe
        print(final_test_name)
